...
```{r ans12}
group_by(classSurvey,性別) %>%
  summarise(人數=n(),參加團隊活動人數=sum(團隊活動=="TRUE"),參加團隊活動比例=sum(團隊活動=="TRUE")/n())
```
...
```{r ans17}
group_by(hwGrades,性別) %>%
  summarise(人數=n(),平均作業平均成績=mean(作業平均成績))
```
