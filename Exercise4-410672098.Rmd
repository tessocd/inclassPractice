...
```{r ans14}
part<-gsHW2ws[,1:5]
gsHW2ws$'答對率'<-rowSums(part,na.rm = T)/5
```
...
```{r ans21}
library(stringr)
allBankData$年月<-str_replace(allBankData$年月,"/","")
allBankData$年月<-as.numeric(allBankData$年月)
allBankData$年月<-((allBankData$年月+191100)*100+1)
library(lubridate)
