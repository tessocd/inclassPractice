...
```{r ans12}
library(dplyr)
mutate(subsample,及格= ifelse(學期成績 >= 60,'True','False')) ->subsample
summarise(subsample,總修習學分數=sum(學分數),總實得學分數=sum(及格=='True')) -> score
```
...
```{r ans13}
summarise(subsample,總修習學分數=sum(學分數),總實得學分數=sum(及格=='True'))
```
...
```{r ans16}
which(transcriptData$學年=="100"  & transcriptData$學期=="1") -> a
transcriptData[a,] -> edu
edu[edu$`必選修類別（必∕選∕通）` =="通",1:10] ->edu
通識科目總表 <- unique(edu[,5])
```
