...
```{r ans11}
factor(classData$`公-10-06`)
str(classData$`公-10-06`)
classData$`公-10-06`<-gsub("1", replacement = "有看公告", classData$`公-10-06`)
classData$`公-10-06`<-gsub("0", replacement = "沒看公告", classData$`公-10-06`)
```
...
```{r ans12}
see<-classData[classData$`公-10-06`=="有看公告",]
```
...
```{r ans14}
nosee<-classData[classData$`公-10-06`=="沒看公告",]
a<-table(nosee$`練2018-10-17-1`=="1")
a[2]/sum(a)
```
...
```{r ans22}
hwData[,3]=array(rep(""),nrow(hwData))
colnames(hwData)<-c("Timestamp","作業代碼","deadline")
hwData$deadline<-as.POSIXct("2018-10-24 00:00:00")
```

### 2.3
將`deadline`減去`hwData$Timestap`你會得到一個向量物件，其class為difftime。它的值為兩個Date/Time差了多少秒,但這並不容易理解。使用`difftime()`算出每個作業交件提早了多少**天**, 並存在``裡。
hwData$early
```{r ans23}
hwData[,4]=array(rep(""),nrow(hwData))
colnames(hwData)<-c("Timestamp","作業代碼","deadline","early")
